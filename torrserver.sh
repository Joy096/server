...
install_torrserver() {
    ...
    if [[ -f "$settings_file" ]]; then
        ...
        echo "Адрес сервера: http://$(curl -s ifconfig.me):$web_port" # Убрали 🌍
...
uninstall_torrserver() {
    echo ""
    echo "Удаление TorrServer..." # Убрали 🚀
...
manage_auth() {
    ...
    while true; do
        echo ""
        echo " Меню авторизации:" # Убрали 🔐
        echo "1. Добавить пользователя" # Убрали 1️⃣
        echo "2. Удалить пользователя" # Убрали 2️⃣
        echo "3. Отобразить всех пользователей" # Убрали 3️⃣
        echo "4. Отключить авторизацию" # Убрали 4️⃣
        echo "0. Назад" # Убрали 0️⃣
        echo "=============================="
...
add_user() {
    ...
    if jq -e --arg user "$answer_user" '.[$user] != null' "$accs_db" >/dev/null; then
        ...
        echo "Такое имя уже существует. Реквизиты для '$answer_user': '$auth_pass'"
        return
    fi

    # Добавляем пользователя и пароль в JSON
    jq --arg user "$answer_user" --arg pass "$answer_pass" '.[$user] = $pass' "$accs_db" > "${accs_db}.tmp" && mv "${accs_db}.tmp" "$accs_db"
...
remove_user() {
    ...
    echo "Список пользователей:" # Убрали 📜
    local users
    ...
    echo -en "\nВведите номер пользователя для удаления: " # Убрали 📝
    read -r num
...
while true; do
    ...
    echo "1. Установка TorrServer" # Убрали 1️⃣
    echo "2. Удаление TorrServer" # Убрали 2️⃣
    echo "3. Авторизация" # Убрали 3️⃣
    echo "4. Очистка кэша" # Убрали 4️⃣
    echo "5. Показать адрес сервера" # Убрали 5️⃣
    echo "0. Выход" # Убрали 0️⃣
    echo "=============================="
    read -p " Введите номер действия (0-5): " choice # Изменили диапазон
    case $choice in
